...
function processData(data){
	...
	dataSet.chart.settings.opacity = 0.25;
	dataSet.chart.settings.yAxis = {};
	dataSet.chart.settings.yAxis.min = null,
	dataSet.chart.settings.yAxis.max = null;
	dataSet.chart.settings.yAxis.minOverride = urlData.minYOverride,
	dataSet.chart.settings.yAxis.maxOverride = urlData.maxYOverride;
...
function createHighchart(graphType){
	...
	FT.interactive.options = {
		chart:{
			...
			marginBottom: dataSet.isSingleDataSet === true ? 50 : 85,
			marginRight: 5
		},
		title:{
		...
		credits:{
			enabled: true,
			text: urlData.sourcetext ? 'Sources: ' + urlData.sourcetext : '',
	...
	FT.interactive.options.tooltip = {
		...
		formatter: function(e){
			...
			if(Array.isArray(this.total)){
				...
				var indexOfPeriod = String(this.total[1]).indexOf('.');

				if(indexOfPeriod > 0){
					if(this.total[1].length - indexOfPeriod > 3){
						//console.log('long decimal');
						this.total[1] = this.total[1].substring(0, indexOfPeriod + 4);
						this.total[1] = Number(this.total[1]);
					}
				}

				if(this.total[2]){
					series = '<b>' + this.total[2] + '</b><br/>';
				}

				return series + date + '<b>' + urlData.unitsBefore + prettyNumber(this.total[1]) + urlData.unitsAfter + '</b>';
			}else{
				if(!this.percentage){
					console.log('this is not an array and does not have a percentage', this);
					return this.key + ': <b>' + urlData.unitsBefore + prettyNumber(this.y) + urlData.unitsAfter + '</b>';
				}else{
					console.log('this is not an array and has a percentage - does it look right?', this);
					return this.key + ': <b>' + urlData.unitsBefore + prettyNumber(this.y) + urlData.unitsAfter + '</b>';
				}
			}
		}
	...
	if(dataSet.isSingleDataSet === true){
		...
		var section = {
			...
			color: dataSet.chart.observation[i].graphType === 'column' || dataSet.chart.observation[i].graphType === 'bar' ? dataSet.chart.settings.colors.bar[i] : dataSet.chart.settings.colors.line[i],
			shadow: false,
			connectNulls: false
		};
		...
		//for(var i = 0; i < dataSet.chart.observation.length; i++){
			dataSet.chart.observation[i].value = String(dataSet.chart.observation[i].value).replace(/[^0-9\-.]/g, '');

			var date = dataSet.chart.observation[i].date;
			var value = Number(dataSet.chart.observation[i].value);
			var dateArray = date.split('-');

			if(dataSet.chart.observation[i].value === null || dataSet.chart.observation[i].value === undefined || String(Number(dataSet.chart.observation[i].value)) === 'NaN'){
				value = null;
			}
		...
		var difference = dataSet.chart.settings.yAxis.max - dataSet.chart.settings.yAxis.min;

		dataSet.chart.settings.yAxis.max = dataSet.chart.settings.yAxis.max + (difference / 6);

		//console.log('min for y: ', dataSet.chart.settings.yAxis.min);
		//console.log('max for y: ', dataSet.chart.settings.yAxis.max);
		...
		dataSet.chart.settings.yAxis.min = undefined;
		...
		for(var i = 0; i < dataSet.chart.observation.length; i++){
			...
			//console.log(dataSet.chart.observation[i]);

			for(var j = 0; j < dataSet.chart.observation[i].data.length; j++){
				
				dataSet.chart.observation[i].data[j].y = Number(String(dataSet.chart.observation[i].data[j].y).replace(/[^0-9\-.]/g, ''));

				if(dataSet.chart.observation[i].data[j].y === null || dataSet.chart.observation[i].data[j].y === undefined || String(Number(dataSet.chart.observation[i].data[j].y)) === 'NaN'){
					value = null;
				}else{
					value = dataSet.chart.observation[i].data[j].y;
				}

				if(dataSet.chart.settings.yAxis.min === undefined){
					dataSet.chart.settings.yAxis.min = value;
				}else if(value < dataSet.chart.settings.yAxis.min){
					dataSet.chart.settings.yAxis.min = value;
				}

				if(dataSet.chart.settings.yAxis.min < 0){
					dataSet.chart.settings.yAxis.min = null;
				}

				if(!dataSet.chart.settings.yAxis.max){
					dataSet.chart.settings.yAxis.max = value;
				}else if(dataSet.chart.settings.yAxis.max < value){
					dataSet.chart.settings.yAxis.max = value;
				}
				...
				if(!$.isNumeric(dataSet.chart.observation[i].data[j].x) && dataSet.chart.observation[i].data[j].x.indexOf('/') === -1 && dataSet.chart.observation[i].data[j].x.indexOf('-') === -1){ //a category
					if(dataSet.chart.observation[i].data[j].y === null || dataSet.chart.observation[i].data[j].y === undefined || String(Number(dataSet.chart.observation[i].data[j].y)) === 'NaN'){
						...
						value = Number(dataSet.chart.observation[i].data[j].y);
					}
		...
		//console.log('lowest y:', dataSet.chart.settings.yAxis.min);
	...
	function applyTheme(){
		...
		Highcharts.theme = {
			...
			yAxis: {
				title: '',
				min: dataSet.chart.settings.yAxis.minOverride ? dataSet.chart.settings.yAxis.minOverride : dataSet.chart.settings.yAxis.min,
				max: dataSet.chart.settings.yAxis.maxOverride ? dataSet.chart.settings.yAxis.maxOverride : dataSet.chart.settings.yAxis.max,
			...
			legend:{
				...
				layout: 'horizontal',
				width: parseFloat(urlData.width) - 20,
...
if(document.URL.indexOf('?') > -1){	
	...
	for(var i = 0; i < arr.length; i++){
		...
		}else if(arr[i].indexOf('y0m1i2n3') > -1){
			if(arr[i].split('y0m1i2n3').join('').substring(1) === '0' || Number(arr[i].split('y0m1i2n3').join('').substring(1)) !== 0){
				urlData.minYOverride = Number(arr[i].split('y0m1i2n3').join('').substring(1));
			}else{
				urlData.minYOverride = null;
			}
		}else if(arr[i].indexOf('y0m1a2x3') > -1){
			if(arr[i].split('y0m1a2x3').join('').substring(1) === '0' || Number(arr[i].split('y0m1a2x3').join('').substring(1)) !== 0){
				urlData.maxYOverride = Number(arr[i].split('y0m1a2x3').join('').substring(1));
			}else{
				urlData.maxYOverride = null;
			}
		}
...
function interpretDate(d){ // yyyy/mm/dd or yyyy/mm or mm/dd/yyyy
	...
	if(p.length > 2){ //full date
		...
		if(p[2].length === 4){
			d = months[Number(p[0]) - 1] + ' ' + Number(p[1]) + ', ' + p[2]; //month, day, year
		}else{
			d = months[Number(p[1]) - 1] + ' ' + Number(p[2]) + ', ' + p[0]; //year, month, day
		}
